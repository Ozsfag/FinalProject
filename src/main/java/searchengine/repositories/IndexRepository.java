...
public interface IndexRepository extends JpaRepository<IndexModel, Integer> {
    ...
    @Transactional(isolation = Isolation.READ_COMMITTED)
    @Query("select i from IndexModel i JOIN FETCH i.page where i.page.id = :pageId and i.lemma.id in = :lemmas")
    List<IndexModel> findByPage_IdAndLemmaIn(@Param("pageId") Integer pageId, @Param("lemmas") Collection<LemmaModel> lemmas);
